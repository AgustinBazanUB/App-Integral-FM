...
export default function ManagementShell({ children }) {
  ...
  const availableRoutes = useMemo(
    () =>
      managementRoutes.filter((route) =>
        canAccessManagementRoute(profile, route.id),
      ),
    [profile],
  );
    ...
    <div className={`fm-app-shell ${collapsed ? "is-collapsed" : ""}`}>
      ...
      {drawerOpen ? (
        <div className="fm-mobile-sidebar-overlay" role="presentation" onMouseDown={(event) => event.target === event.currentTarget && setDrawerOpen(false)}>
          <aside className="fm-mobile-sidebar" role="dialog" aria-modal="true" aria-label="Navegación">
            <IconButton label="Cerrar menú" icon="X" className="fm-mobile-sidebar__close" onClick={() => setDrawerOpen(false)} />
            {renderNavigation()}
          </aside>
        </div>
      ) : null}
      ...
      <header className="fm-management-header">
        <div className="fm-management-header__leading">
          <IconButton label="Abrir menú" icon="Menu" className="fm-mobile-menu-trigger" onClick={() => setDrawerOpen(true)} />
      ...
      <div className="fm-live-region" aria-live="polite">{online ? "Conexión disponible" : "Sin conexión. No se podrán confirmar operaciones."}</div>
    </div>
  );
}
