...
export default function ProductsPage() {
  ...
  const filtered = useMemo(() => {
    ...
    return products.filter((product) => {
      if (term && !`${product.name || ""} ${product.productCode || product.abbreviation || ""} ${product.description || ""}`.toLocaleLowerCase("es").includes(term)) return false;
            ...
            {filtered.map((product) => (
              <article className="fm-inventory-card" key={product.id}>
                <header className="fm-inventory-card__identity">
                  ...
                  <div>
                    <h3>{product.name}</h3>
                    <p>{product.categoryName || "Sin categoría"}{(product.productCode || product.abbreviation) ? ` · ID del producto: ${product.productCode || product.abbreviation}` : ""}</p>
                  </div>
