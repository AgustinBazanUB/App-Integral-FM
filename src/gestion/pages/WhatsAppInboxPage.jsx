...
import {
  ...
  sendWhatsAppInboxText,
} from "../social/whatsapp/inboxBridge";
...
function emptyStateForConnection(connection, retry) {
  if (connection.code === "EXTENSION_NOT_AVAILABLE" || connection.code === "extension_unavailable") {
    return <EmptyState icon="PlugZap" title="La extensión no está disponible" description="Abrí esta Web App en un dominio autorizado y comprobá que Flor Mía WhatsApp Sender esté instalada y habilitada." action={<Button variant="secondary" onClick={retry}>Volver a comprobar</Button>} />;
  }
  if (connection.code === "WHATSAPP_NOT_OPEN") {
    return <EmptyState icon="MessagesSquare" title="WhatsApp Web está cerrado" description="Abrí web.whatsapp.com en otra pestaña con esta misma sesión de Chrome y después reintentá." action={<Button variant="secondary" onClick={retry}>Ya lo abrí</Button>} />;
  }
  if (connection.code === "SESSION_NOT_READY" || connection.code === "session_not_ready") {
    return <EmptyState icon="QrCode" title="WhatsApp necesita iniciar sesión" description="Completá el inicio de sesión en WhatsApp Web. La bandeja no simula conversaciones mientras la sesión no esté disponible." action={<Button variant="secondary" onClick={retry}>Comprobar sesión</Button>} />;
  }
...
function zoneSelection(customer, zones) {
  ...
  return {
    name: customer.name || "",
    zoneId: custom ? "__custom" : (configured?.id || ""),
    customZone: custom,
  };
}

function displayedPhone(chat) {
  if (!chat?.phone) return "Teléfono no expuesto por WhatsApp";
  return formatPhoneForDisplay(normalizeCustomerPhone(chat.phone)) || chat.phone;
}

function CustomerBadge({ crm }) {
...
function ChatList({ chats, selectedId, crmByChat, onSelect }) {
  return (
    <div className="fm-wa-inbox__chat-list" role="list">
      {chats.map((chat) => {
        ...
        return (
          <button
            ...
            role="listitem"
            key={chat.chatId}
            className={`fm-wa-chat ${selectedId === chat.chatId ? "is-selected" : ""}`}
          ...
          >
            ...
            <span className="fm-wa-chat__body">
              ...
              <span className="fm-wa-chat__meta">
                ...
                {!chat.phone ? <span><Icon name="ShieldQuestion" />Teléfono no disponible</span> : null}
              </span>
            </span>
            {chat.unreadCount > 0 ? <span className="fm-wa-chat__unread" aria-label={`${chat.unreadCount} mensajes no leídos`}>{chat.unreadCount}</span> : null}
          </button>
        );
      })}
    </div>
...
function Messages({ conversation }) {
  if (!conversation?.messages?.length) {
    return <EmptyState icon="MessageCircle" title="No hay mensajes de texto visibles" description="Esta primera versión muestra únicamente mensajes de texto recientes. Multimedia no se importa ni se almacena." />;
  }
  return (
    <div className="fm-wa-conversation__messages" aria-live="polite">
      {conversation.hasMore ? <div className="fm-wa-conversation__history-note">Hay historial anterior. Esta etapa carga sólo el bloque reciente para evitar historiales enormes.</div> : null}
      {conversation.messages.map((message) => (
        <article key={message.messageId} className={`fm-wa-message fm-wa-message--${message.direction}`}>
          <p>{message.text}</p>
          {message.timestampLabel ? <small>{message.timestampLabel}</small> : null}
        </article>
      ))}
    </div>
  );
...
export default function WhatsAppInboxPage() {
  ...
  const hydrateCrm = useCallback(async (nextChats) => {
    ...
    const entries = await Promise.all(nextChats.map(async (chat) => {
      const normalized = normalizeCustomerPhone(chat.phone);
      if (!normalized) return [chat.chatId, { status: "unavailable", customer: null }];
      let operation = lookupCache.current.get(normalized);
      if (!operation) {
        operation = findCustomerByPhone(normalized).catch((error) => ({ __lookupError: error }));
        lookupCache.current.set(normalized, operation);
      }
  ...
  const selectChat = useCallback(async (chat) => {
    ...
    try {
      ...
      setSelectedChat(next.chat || chat);
      setChats((current) => current.map((item) => item.chatId === chat.chatId ? { ...item, unreadCount: 0, ...(next.chat || {}) } : item));
  ...
  const visibleChats = useMemo(() => {
    ...
    return chats.filter((chat) => {
      ...
      const haystack = [
        ...
        chat.lastMessage,
        crm?.customer?.name,
        crm?.customer ? customerZoneLabel(crm.customer) : "",
  ...
  const saveCrm = async () => {
    ...
    try {
      ...
      if (selectedCrm?.status === "existing") {
        ...
        customerId = await saveCustomerFromAdmin(profile, input);
      }
      lookupCache.current.delete(normalizeCustomerPhone(selectedChat.phone));
      ...
      setCrmMessage(selectedCrm?.status === "existing" ? "Cliente actualizado en CRM." : "Cliente creado sin duplicar el teléfono.");
    } catch (error) {
  ...
  const sendText = async (event) => {
    ...
    if (!selectedChat || !draft.trim() || sendState.status === "sending") return;
    const text = draft.trim();
    setSendState({ status: "sending", message: "Enviando…" });
    try {
      const result = await sendWhatsAppInboxText(selectedChat.chatId, text);
      setDraft("");
      setSendState({ status: "sent", message: result?.verified === false ? "Enviado; WhatsApp no expuso una confirmación visual fuerte." : "Enviado" });
      const refreshed = await getWhatsAppInboxMessages(selectedChat.chatId, { limit: 50 });
      setConversation(refreshed);
    } catch (error) {
      setSendState({ status: "error", message: error.message || "No se pudo enviar el mensaje." });
    }
  };

  const connectionUi = connectionPresentation(connection);
  const selectedLabels = [
    ...(Array.isArray(selectedChat?.labels) ? selectedChat.labels : []),
    ...(Array.isArray(selectedCrm?.customer?.tags) ? selectedCrm.customer.tags : []),
  ].filter(Boolean);

  return (
    <div className="fm-page fm-wa-inbox-page">
      <PageHeader
        eyebrow="Redes Sociales · WhatsApp"
        title="Bandeja comercial"
        description="Respondé conversaciones de texto y completá la ficha comercial sin copiar el historial de WhatsApp a Firebase."
        actions={<div className="fm-wa-inbox__header-actions"><Badge tone={connectionUi.tone}>{connectionUi.label}</Badge><Button variant="secondary" icon="RefreshCw" onClick={loadInbox}>Actualizar</Button></div>}
      />
      ...
      {connection.status === "error" ? (
        <section className="fm-wa-inbox__connection-state">{emptyStateForConnection(connection, loadInbox)}</section>
      ) : (
        <div className="fm-wa-inbox" data-mobile-view={mobileView}>
          <aside className="fm-wa-inbox__list-column">
            <div className="fm-wa-inbox__toolbar">
              <SearchInput label="Buscar conversaciones" value={search} onChange={(event) => setSearch(event.target.value)} />
              <div className="fm-wa-inbox__filters" role="tablist" aria-label="Filtros de conversaciones">
                {FILTERS.map((item) => <button type="button" role="tab" aria-selected={filter === item.id} key={item.id} className={filter === item.id ? "is-active" : ""} onClick={() => setFilter(item.id)}><span>{item.label}</span><b>{counts[item.id]}</b></button>)}
              </div>
            </div>
          ...
          <main className="fm-wa-inbox__conversation-column">
            {!selectedChat ? <EmptyState icon="MessageSquareText" title="Elegí una conversación" description="Los mensajes se solicitan a WhatsApp Web sólo cuando abrís un chat; no se descarga el historial completo." /> : (
              <>
                <header className="fm-wa-conversation__header">
                  <button type="button" className="fm-wa-inbox__back" onClick={() => setMobileView("list")} aria-label="Volver a chats"><Icon name="ArrowLeft" /></button>
                  <div><strong>{selectedChat.name || displayedPhone(selectedChat)}</strong><span>{displayedPhone(selectedChat)}</span></div>
                  <CustomerBadge crm={selectedCrm} />
                  <button type="button" className="fm-wa-inbox__open-crm" onClick={() => setMobileView("crm")}><Icon name="ContactRound" />Ficha</button>
                </header>
                ...
                <form className="fm-wa-composer" onSubmit={sendText}>
                  <label htmlFor="wa-inbox-reply">Responder por WhatsApp</label>
                  <div><textarea id="wa-inbox-reply" rows="2" value={draft} maxLength={4096} onChange={(event) => setDraft(event.target.value)} placeholder="Escribí un mensaje de texto…" /><Button type="submit" icon="Send" loading={sendState.status === "sending"} disabled={!draft.trim()}>Enviar</Button></div>
                  {sendState.message ? <small className={`fm-wa-composer__status fm-wa-composer__status--${sendState.status}`}>{sendState.message}</small> : null}
                </form>
              </>
            )}
          </main>
          ...
          <aside className="fm-wa-inbox__crm-column">
            {!selectedChat ? <EmptyState icon="ContactRound" title="Ficha comercial" description="Seleccioná un chat para relacionarlo con Clientes." /> : (
              <div className="fm-wa-crm-card">
                ...
                <CustomerBadge crm={selectedCrm} />
                <dl className="fm-wa-crm-card__facts">
                  <div><dt>Teléfono</dt><dd>{displayedPhone(selectedChat)}</dd></div>
                  <div><dt>Zona</dt><dd>{selectedCrm?.customer ? customerZoneLabel(selectedCrm.customer) || "Sin asignar" : "Sin asignar"}</dd></div>
                </dl>

                {!canViewCustomers ? <Toast tone="warning">Tu perfil puede atender Redes Sociales, pero no tiene permiso para consultar Clientes.</Toast> : null}
                {selectedCrm?.status === "error" ? <Toast tone="error">No se pudo consultar el cliente por teléfono.</Toast> : null}
                {selectedCrm?.status === "unavailable" ? <Toast tone="warning">WhatsApp no expuso un teléfono verificable para este chat. La conversación puede responderse, pero no se relaciona automáticamente con CRM.</Toast> : null}

                {(selectedCrm?.status === "existing" || selectedCrm?.status === "new") ? (
                  <div className="fm-wa-crm-card__form">
                    ...
                    <FormField label="Zona" required hint="Se guarda en la misma ficha de Clientes y queda disponible para segmentación.">
                      <select value={crmForm.zoneId} onChange={(event) => setCrmForm((current) => ({ ...current, zoneId: event.target.value, customZone: event.target.value === "__custom" ? current.customZone : "" }))} disabled={selectedCrm.status === "existing" ? !canEditCustomers : !canCreateCustomers}>
                    ...
                    {crmForm.zoneId === "__custom" ? <FormField label="Nueva zona para este cliente"><input value={crmForm.customZone} onChange={(event) => setCrmForm((current) => ({ ...current, customZone: event.target.value }))} disabled={selectedCrm.status === "existing" ? !canEditCustomers : !canCreateCustomers} /></FormField> : null}
                ...
                <section className="fm-wa-crm-card__labels">
                  <h3>Etiquetas</h3>
                  {selectedLabels.length ? <div>{[...new Set(selectedLabels)].map((label) => <Badge key={label} tone="neutral">{label}</Badge>)}</div> : <p>Sin etiquetas compatibles detectadas. Esta etapa no crea un segundo sistema de etiquetas.</p>}
                </section>
